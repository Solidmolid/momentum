...
function appSecurityApi() {
  ...
  const values = new Map([
    ...
    ["momentum_conflict_backup::user-1", "conflict"],
    ["unrelated", "keep"],
  ]);
  const localStorage = {
  ...
  const currentRequests = [
    { url: "http://127.0.0.1:8124/app.js?v=31" },
    { url: "https://uytacdogqercenlgbpgb.supabase.co/rest/v1/user_states" },
  ];
  const caches = {
    async keys() { return ["momentum-v22", "momentum-v23", "momentum-v24", "momentum-v25", "momentum-v26", "momentum-v27", "momentum-v28", "momentum-v29", "momentum-v30", "momentum-v31", "another-app-v1"]; },
    async delete(name) { deletedCaches.push(name); return true; },
    async open(name) {
      assert.equal(name, "momentum-v31");
      return {
...
async function testStateSanitizingAndLogoutPurge() {
  ...
  assert.deepEqual(deletedCaches, ["momentum-v22", "momentum-v23", "momentum-v24", "momentum-v25", "momentum-v26", "momentum-v27", "momentum-v28", "momentum-v29", "momentum-v30"]);
...
function testServiceWorkerCacheBoundary() {
  ...
  assert.equal(isIntercepted("https://solidmolid.github.io/momentum/app.js?v=31"), true);
